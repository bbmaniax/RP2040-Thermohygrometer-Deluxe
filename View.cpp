...
void View::renderAllText() {
  ...
  display1.setRotation(flipped ? 2 : 0);
  display1.setTextSize(2);
  display1.setTextColor(SSD1306_WHITE);
  display1.setCursor(0, 0);
  printRight(display1, String(model.getLatestTemperature() / 10.0f, 1), 0);

  display2.clearDisplay();
  display2.setRotation(flipped ? 2 : 0);
  display2.setTextSize(2);
  display2.setTextColor(SSD1306_WHITE);
  display2.setCursor(0, 0);
  printRight(display2, String(model.getLatestHumidity() / 10.0f, 1), 0);

  display3.clearDisplay();
  display3.setRotation(flipped ? 2 : 0);
  display3.setTextSize(2);
  display3.setTextColor(SSD1306_WHITE);
  display3.setCursor(0, 0);
  printRight(display3, String(model.getLatestPressure() / 10.0f, 1), 0);
...
void View::printRight(SSD1306Display& display, const String& text, int16_t y) {
  ...
  w = text.length() * 6 * display.getWidth() / 128;
...
void View::renderAllCharts() {
  ...
  renderTemperatureChart(display1);
  renderHumidityChart(display2);
  renderPressureChart(display3);
}
